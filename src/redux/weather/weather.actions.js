...
export const fetchDailyReadingStartAsync = (city = "Cagayan de Oro") => {
	return async (dispatch) => {
	 	try{
	 		...
	 		const fetchData = await fetch(`http://api.openweathermap.org/data/2.5/forecast?q=${encodeURI(city)}&units=imperial&APPID=${process.env.REACT_APP_OPEN_WEATHER_API_KEY}`);
